...
This is the essence of this proposal: to modify the partial-generating XSLT to create static HTML pages for the /Catalogo/ and its entries.  The site would comprise a series of pages containing sections of the /Catalogo/ (like the current site), plus single pages for each entry, each page containing calculated facets and code to run a IIIF viewer.  Some search capacity might be provided through [[https://lunrjs.com/][Lunr]], a Javascript library commonly used to build and search indexes of static sites.  To make the site even more robust and future-proof, the site-building software would pre-build the indexes used by Lunr.

*** Static Pages
- /Catalogo/ The /Catalogo/, like many early-modern books, is elaborately partitioned into sections and subsections. It comprises two printed volumes approximately 4,800 distinct items in 45 sections.
  
  - Section pages :: the current UI presents a linked table of contents pointing to discrete HTML renditions of each section (each section has its own restful URI). Each of these could be a stand-alone HTML document.
