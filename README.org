...
The TEI-encoded edition of Cicognara's /Catalogo/ is an under-appreciated resource.  Like other deep encodings of primary-source materials, it is both a document and a database.  PUL itself has no interest in providing tools for exploiting encoded texts, but simply making it available as an XML file for others to use in research would be entirely in keeping with the Library's mission.
